...
function StatesPage() {
  const { states, setSelectedState } = useContext(configContext)
      ...
      <div className='overflow-y-scroll pt-14 h-screen '>
        <ul className='w-screen flex flex-col items-center px-6 '>
          {Object.entries(states).map(([key, value]) => {
            return (
              <li className={`fade-up bg-gray-50   text-lime-950 text-lg font-semibold w-full shadow shadow-blue-500/40 hover:shadow-indigo-500/40 py-5  rounded-md mb-2 text-center }`} onClick={onStateChange.bind(this, value)} key={key}>
                {value}
              </li>
            )
          })}
