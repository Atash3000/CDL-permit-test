      ...
      <div className='overflow-y-scroll pt-14 h-screen '>
        <ul className='w-screen flex flex-col items-center px-6 '>
          {Object.entries(states).map(([key, value]) => {
            ...
            return (
              <li className={`fade-up bg-lime-50 text-lime-950 text-lg font-semibold w-full  py-4  rounded-md mb-2 text-center ${isSelected ? 'selected' : 'unselected'}`} onClick={onStateChange.bind(this, key)} key={key}>
                {value}
              </li>
            )
          })}
